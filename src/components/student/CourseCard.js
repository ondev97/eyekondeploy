...
export default function CourseCard({
  ...
  const clk = () => {
    Axios.post(
      `${process.env.REACT_APP_LMS_MAIN_URL}/course-api/enrollcourse/${courseid}/${usDetails.id}/`,
      {
        coupon_key: key,
      },
      {
  ...
  const freeEn = () => {
    Axios.post(
      `${process.env.REACT_APP_LMS_MAIN_URL}/course-api/freenroll/${courseid}/${usDetails.id}/`,
      {},
      {
        headers: { Authorization: "Token " + usDetails.key },
      }
